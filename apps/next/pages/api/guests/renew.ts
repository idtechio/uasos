...
async function renewGuest(
  ...
  try {
    ...
    if (!guest) {
      throw new Error("Guest's offer does not exist");
    }

    const topicNameOrId = process.env.TOPIC_GUEST_RENEW;
...
export default withApiAuth(renewGuest);
