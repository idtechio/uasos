...
export const Grid = styled.div<{spaceing:[number, number], childrenCount:number, mobileReverse:boolean, alignItems:string, disableRwd:boolean}>`
	display: flex;
	flex-wrap: wrap;
	...
	: null}
	& > div {
		width: calc(${(props)=>100/props.childrenCount}% - ${(props)=>props.spaceing ? props.spaceing[0] : 0}px);
		margin-left: ${(props)=>props.spaceing ? props.spaceing[0]/2 : 0}px;
		margin-right: ${(props)=>props.spaceing ? props.spaceing[0]/2 : 0}px;
		margin-bottom: ${(props)=>props.spaceing ? props.spaceing[1] : 0}px;
		${(props)=>!props.disableRwd ? 
