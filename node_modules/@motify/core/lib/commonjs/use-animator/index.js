...
exports.default = useAnimationState;
 ...
 */
function useAnimationState(_variants, {
  from = 'from',
  to = 'to'
} = {}) {
  const controller = (0, _react.useRef)();

  const __state = (0, _reactNativeReanimated.useSharedValue)(from ? _variants[from] : 0);

  const selectedVariant = (0, _react.useRef)(from);
  const variants = (0, _react.useRef)(_variants);
  (0, _react.useEffect)(function updateVariantsRef() {
  ...
  (0, _react.useEffect)(function maybeTransitionOnMount() {
    if (variants.current[to]) {
      if (variants.current[from]) {
        var _controller$current;

        (_controller$current = controller.current) === null || _controller$current === void 0 ? void 0 : _controller$current.transitionTo(to);
      } else {
        console.error("\uD83D\uDC3C [".concat(_constants.PackageName, "]: Called useAnimationState with a \"to\" variant, but you are missing a \"from\" variant. A \"from\" variant is required if you are using \"to\". Instead, you passed these variants: \"").concat(Object.keys(variants.current).join(', '), "\". If you want to just use the \"to\" value without \"from\", you shouldn't use this hook. Instead, just pass your values to a ").concat(_constants.PackageName, " component's \"animate\" prop."));
      }
    }
  }, [from, to]);
