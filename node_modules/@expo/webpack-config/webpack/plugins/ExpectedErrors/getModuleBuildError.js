...
async function getModuleBuildError(compilation, input) {
    ...
    if (input.name === 'ModuleDependencyWarning') {
        return await parseNotFoundError_1.getModuleDependencyWarning(compilation, input, sourceFilename);
    }
    const notFoundError = await parseNotFoundError_1.getNotFoundError(compilation, input, sourceFilename);
    if (notFoundError !== false) {
        return notFoundError;
    }
    const babel = parseBabelError_1.getBabelError(sourceFilename, input.error);
    if (babel !== false) {
        return babel;
    }
    return false;
